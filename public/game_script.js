...
var categoriesGuessedCorrectly = 0;
var level;
const maxLevel = 3;
var shuffledWords = [];
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  level = localStorage.getItem("currentLevel") || 1;
  getUserInformation();
  unauthorized();
  fetchDatabase(level);
  ...
  check_selections.addEventListener("click", function () {
    const selectedInputs = document.querySelectorAll(
      "#game_container .letter.selected input"
    );
  ...
  shuffle_words.addEventListener("click", () => {
    ...
    const visibleLetterInputs = Array.from(
      document.querySelectorAll("#game_container .row")
    )
...
function updateSelection(letter) {
  const selectedLetters = document.querySelectorAll(
    "#game_container .letter.selected"
  );
...
function checkSelectedWords() {
  ...
  if (maxWordsInCategory < 4) {
    ...
    document
      .querySelectorAll("#game_container .letter input")
      .forEach((input) => {
  ...
  const hiddenLifeBalls = lifeBalls.filter(
  ...
  if (hiddenLifeBalls.length === 4) {
    swal({
      ...
      // Using then() to wait for the swal to close
  ...
  if (categoriesGuessedCorrectly >= 4) {
    if (level >= maxLevel) {
      swal({
        ...
        restart_the_game(); // Restart the game or offer to start a new game
      });
    } else {
      swal({
        title: "Great job!",
        text: `You've cleared level ${level}.`,
        icon: "success",
      }).then(() => {
          console.log("The user clicked the OK button.");
          level++;
          categoriesGuessedCorrectly = 0; // Reset counter for next level
...
function hideRowAndReshuffle(rowNumber) {
  ...
  if (rowToHide && guessedRowToShow) {
    ...
    console.log(
      "No row found to hide or no guessed row to show for row number:",
      rowNumber
    );
  }
}
...
function reshuffleAndReassignWords() {
  ...
  const remainingRows = Array.from(document.querySelectorAll(".row")).filter(
...
function fetchDatabase(level) {
  fetch(`http://localhost:3000/placeholders/${level}`)
    ...
    .then((data) => {
      ...
      data.forEach((item) => {
        categories[item.categoryName] = item.words.map((word) =>
          word.toUpperCase()
        );
